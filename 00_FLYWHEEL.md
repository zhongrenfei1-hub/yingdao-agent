...
| 2026-05-18 | 17 | 阶段 17 本地素材真接入混剪(用户选 a):前端拖入素材 → 渲染时混剪真用用户素材。`vite.config.ts` 加 `assetUploadApiPlugin` /api/assets/upload(busboy multipart,3 类型白名单 mp4/webm/mov/jpg/png/webp,单文件 50MB,落到 `local-asset-remix/assets/uploads/<sid>/`,返回相对路径);改造 `renderViaHyperframes` 接 assetPaths 参数 → 切 `local-asset-remix` composition + 传 c{1,2,3}Url variables(< 3 自动 fallback demo clip)+ scriptToRemixCaptions 把 scene[i].caption 映射到 caption slot。`local-asset-remix/index.html` 加 c1Url/c2Url/c3Url 三 variable + setSrc 注入(default 保留 ./assets/clip-N.mp4 不破坏 demo)。`hyperframes lint` ✓ 0/0 仍 pass。`adapters/video-renderer.ts` 加 uploadAssets() + assetPaths 字段。`core/quickMake.ts` 串通。`ui/QuickMakeWorkbench.tsx` 加 AssetUploader 子组件:点击 / 拖拽 / 类型校验 / 大小校验 / 3 槽位 / 缩略图 / 单删 / warnings。`.gitignore` 排除 `assets/uploads/`。`tsc --noEmit` ✓,`vite build` 287 KB / gzip 90 KB ✓ | preview 工具因 worktree 限制不可用,完整链路要 `npm run dev`(port 5180)上传真 mp4 才能验证。busboy 加在 devDep 因为只在 vite dev middleware 用,build 不需 | (pending) |
